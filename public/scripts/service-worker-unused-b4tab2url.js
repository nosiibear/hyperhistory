...
let tabInfoPools = new Map();
let tabsWaiting = new Map();
let messageQueue = []
let vispageActive = true
...
chrome.tabs.onActivated.addListener(async (activeInfo) => {
  console.log("Inside onActive listener, activeInfo:")
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.nodeReady) {
    ...
    sendMsg({updatedTabInfo: {tabId, data: tabInfoPools.get(message.nodeReady)}})
    tabsWaiting.delete(message.nodeReady);
    tabInfoPools.delete(message.nodeReady);
  }
});
    ...
    if(changeInfo.url) {
      ...
      // changed info, waiting for a signal back that the node is finished being made
        ...
        if(tabsWaiting.has(tab.id)) {
          ...
          if(!tabInfoPools.has(tab.id)) {
            tabInfoPools.set(tab.id, {});
          }
          tabInfoPools.set(tab.id, {...tabInfoPools.get(tab.id), ...leftoverInfo})
          console.log("tabInfoPools")
          console.log(tabInfoPools)
          console.log("tabsWaiting")
          console.log(tabsWaiting)
        } else {
          ...
          sendMsg({updatedTabInfo: {tabId, data: leftoverInfo}});
        }
      }
    }
  }
