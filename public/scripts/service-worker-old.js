...
chrome.action.onClicked.addListener(async (tab) => {
  ...
  const existingUrl = await chrome.storage.local.get('visPageUrl');
  console.log("existingUrl");
  console.log(existingUrl)
  console.log(existingUrl.visPageUrl)
  if(existingUrl.visPageUrl) {
    const query = await chrome.tabs.query({url: existingUrl.visPageUrl})
    const existingId = query[0].id
    chrome.tabs.update(existingId, {'active': true}, () => console.log("Opened existing vispage"))
  } else {
    console.log("Opening new vispage")
    const url = chrome.runtime.getURL("index.html")
    chrome.tabs.create({ url: url });
    chrome.storage.local.set({visPageUrl: url})
...
chrome.tabs.onActivated.addListener(async () => {
  ...
  if(existingUrl.visPageUrl) {
    console.log(existingUrl.visPageUrl)
    const query = await chrome.tabs.query({url: existingUrl.visPageUrl})
    //chrome.runtime.message.send({activeVispage})
  }
});
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.nodeReady) {
    const tabId = message.nodeReady;
    console.log(`%cNode ready, tab id: ${tabId}`, 'background-color: green; color: white')
    chrome.runtime.sendMessage({updatedTabInfo: {tabId, data: tabInfoQueues.get(message.nodeReady)}})
    tabsWaiting.delete(message.nodeReady);
    tabInfoQueues.delete(message.nodeReady);
  }
});
  ...
  if(!url.startsWith("chrome")) {
    if(changeInfo.url) {
      ...
      // changed info, waiting for a signal back that the node is finished being made
        ...
        if(tabsWaiting.has(tab.id)) {
          console.log("Tab is waiting, adding info to queue instead of sending");
          if(!tabInfoQueues.has(tab.id)) {
            tabInfoQueues.set(tab.id, {});
          }
          tabInfoQueues.set(tab.id, {...tabInfoQueues.get(tab.id), ...leftoverInfo})
          console.log(tabInfoQueues.get(tab.id))
        } else {
...
chrome.tabs.onRemoved.addListener((tabId, removeInfo) => {
  ...
  chrome.storage.local.set({visPageUrl: null})
});